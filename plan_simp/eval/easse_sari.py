...
from plan_simp.eval.dsari import count_length, sentence_number
...
def get_corpus_sari_operation_scores(
    ...
    if doc_level:
        ...
        for i in range(len(orig_sents)):
            input_length, reference_length, output_length = count_length(orig_sents[i], sys_sents[i], [ref[i] for ref in refs_sents])
            reference_sentence_number, output_sentence_number = sentence_number(sys_sents[i], [ref[i] for ref in refs_sents])
            ...
            # verbosity penalty
            if output_length > reference_length:
                LP2 = math.exp((reference_length - output_length) / max(input_length - reference_length, 1))
            else:
                LP2 = 1
